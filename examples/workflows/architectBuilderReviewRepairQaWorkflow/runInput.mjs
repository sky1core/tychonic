...
export function validateRunInput(input) {
  validateTaskInput(input, PROMPT_ADDITION_STATES);
}

function validateTaskInput(input, promptAdditionStates) {
  ...
  if (input.promptAdditions !== undefined) validatePromptAdditions(input, promptAdditionStates);
}

function validatePromptAdditions(input, promptAdditionStates) {
  if (!isPlainObject(input.promptAdditions)) throw new Error("promptAdditions must be an object keyed by state name");
  const allowedStates = new Set(promptAdditionStates);
