...
function validateTaskInput(input, contract = {}) {
  ...
  if (allowGoal) allowedFields.add("goal");
  ...
  if (input.promptAdditions !== undefined) {
    validatePromptAdditions(input, contract.promptAdditionStates);
  }
}

function validatePromptAdditions(input, allowedStateNames) {
  if (allowedStateNames === undefined) throw new Error("unsupported input field: promptAdditions");
  if (!isPlainObject(input.promptAdditions)) {
    throw new Error("promptAdditions must be an object keyed by state name");
  }
  const allowedStates = new Set(allowedStateNames);
  const states = isPlainObject(input.profile) && isPlainObject(input.profile.states)
