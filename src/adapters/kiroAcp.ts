/**
 * Kiro ACP built-in adapter (binary `kiro-cli acp`) — PARTIAL: prose review needs a normalizer.
 *
 ...
 *
 * Kiro ACP is an agent/server. The wrapper below acts as the minimal client
 ...
 *
 * Reviewer role can produce prose review output without `--trust-all-tools`,
 * but still needs a host normalizer because Kiro ACP does not provide a
 * Tychonic structured-review contract.
 */
...
const BIN = "kiro-cli";
const SESSION_EXPORT_START = "__TYCHONIC_KIRO_ACP_SESSION_START__";
const SESSION_EXPORT_END = "__TYCHONIC_KIRO_ACP_SESSION_END__";
...
function buildCommand(input: AdapterRunInput, resumeSessionId?: string): string {
  ...
  return [
    'tmpdir=$(mktemp -d "${TMPDIR:-/tmp}/tychonic-kiro-acp.XXXXXX")',
...
function extractSessionId(stdout: string): string | undefined {
  ...
  if (end < 0) {
    throw new Error("kiro-acp session export marker start was present without an end marker");
  }
...
export const kiroAcpAdapter: AgentAdapter = {
  name: "kiro-acp",
...
const KIRO_ACP_CLIENT_SOURCE = String.raw`
...
const child = spawn("kiro-cli", ["acp", ...(trustAllTools ? ["--trust-all-tools"] : [])], {
  cwd: workspaceRoot,
  env: process.env,
...
try {
  ...
  console.log("");
  console.log("__TYCHONIC_KIRO_ACP_SESSION_START__");
  console.log(JSON.stringify({ session_id: sessionId }));
  console.log("__TYCHONIC_KIRO_ACP_SESSION_END__");
...
function printSessionUpdate(params) {
  ...
  if (sessionUpdate === "tool_call_update" || sessionUpdate === "ToolCallUpdate") {
    ...
    console.log("[kiro-acp] tool " + status);
  }
}
