...
- Only a parsed review verdict with `status: "fail"` is failed review feedback.
  In declarative YAML workflows generated by install, a returned review outcome
  with `kind: "command_failed"`, `kind: "unparseable"`, or `kind: "skipped"`
  is an infrastructure/configuration failure; the generated wrapper surfaces it
  as `waiting_user` instead of sending semantic remediation back to
  `on_fail_return_to`.
...
- `ctx.finish` and `ctx.finishWaitingUser` call `extractWorktreePatchActivity`
  ...
  on disk.
- `baseHead` is the isolated worktree baseline returned by
  `createWorktreeActivity`, not necessarily the target repository's original
  HEAD. Files copied from the target repository before workflow work starts are
  part of that baseline and are not workflow output by themselves.
